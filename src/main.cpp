...
#if defined(NDEBUG)
# error "Chosn cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xac2c6f6edcafb690c795fe41d01083074ae1986317965c9cfb6833eec1be2a5e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chosn: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chosn Signed Message:\n";
...
{
    // Chosn: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chosn: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chosn: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1519284331, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Bizarre new sex district rules 22feb2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1337 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d8778eec31d545091f7f9bbf4452a6105b04f4c7b98bd36efd15823ee36d98bd30196fd4c85dd9fe46992512f49e7f6378c42c9844096745b3df57fdc9a17357") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519284331;
        ...
        assert(block.hashMerkleRoot == uint256("0xda5f92ce49f96293f4497e6c20d0c601aebcc43d8c3c45365411b42b49959c29"));
...
unsigned char pchMessageStart[4] = { 0xaa, 0xa3, 0x9a, 0x29 }; // Chosn: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChosnMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChosnMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChosnMiner(CWallet *pwallet)
{
    printf("ChosnMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chosn-miner");
    ...
    try { loop {
        ...
        printf("Running ChosnMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChosnMiner, pwallet));
}
